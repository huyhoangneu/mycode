...
Class GooglePR 
{
	...
	public $googleDomains = Array(
		...
		"www.google.com.tr",
		"toolbarqueries.google.cn",
		"www.google.cn", 
		"64.233.187.99", 
		"72.14.207.99");
	
	public $debugResult = Array();
	public $userAgent = "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.2.1) Gecko/20021204";// 常量
	public $cacheDir = "/tmp";
	public $maxCacheAge = 86400; // = 24h (yes, in seconds)
	...
	function debugRes($what,$sowhat) 
	{
		if($this->debug == true) 
		{
			...
			$what = trim($what);
			//print_r($debugbt);exit;
			$__sowhat = trim($sowhat) . " (Line : ".$debugbt['0']['line'].")";
			if ($what == "error") 
			{
				$this->debugResult[$what][] = $__sowhat;
			} 
			else 
			{
				$this->debugResult[$what] = $__sowhat;
