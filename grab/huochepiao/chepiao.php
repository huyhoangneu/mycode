<?php
$huochepiao_url = 'http://www.huochepiao.com/sell/?Type=all';
$huoche = iconv('gbk', 'utf-8', getHtmlContents($huochepiao_url));
preg_match_all("'· <A\s+href=(.*?)\s+target=_blank>(.*?)</a>'is", $huoche, $block1);
if(!empty($block1['0']))
{
		$i = 0;
		foreach($block1['0'] AS $huoche_k => $huoche_v)
		{
				$info = explode(' ', $block1['2'][$huoche_k]);
				if(count($info) == 5)
				{
						//车次
						$train_number = $info['1'];
						$station = strtr($info['2'], array('(' => '', ')'=> ''));
						$number = preg_replace("'[^\d]+'is", '', $info['3']);
						$t = explode('：', $info['4']);
						$time = $t['1'];
						if($station)
						{
								if($i < 10)
								{
									//$alls['huochepiao.com'][] = array($train_number, $station, $number, $time);
									//$alls['huochepiao.com'][] = array('url' => $block1['1'][$huoche_k], 'info' => $station.' '.$train_number.' '.($number ? $number.'张': ''), 'type' => '--', 'starttime' => $time, 'time' => '1分钟前');
									$i +=1;
								}
						}
				}
...
for($i =1; ;$i++)
{
	...
	foreach($block1['1'] AS $k1 => $v1)
	{
		...
		{
			if(count($alls[$f]) < 10 && $f != 'huochepiao.com')
			{
...
function createTbl($arr, $flag)
{
		...
		foreach($arr AS $k => $v)
		{
			...
			//	print_r($tmp);
				...
				//$html .= '<tr><td><a href="'.$v['url'].'">[转让]'.trim(strtr($tmp['0'], array('发车日期' => ''))).'</a></td><td width="12%" align="center">'.(($k == 'huochepiao.com') ? $v['starttime'] : $tmp['1']).'</td><td width="8%" align="center">'.$v['type'].'</td><td width="11%" align="center">'.$che.'</td><td align="center">'.$v['time'].'</td></tr>';
		}
		//$html .= '<tr><td colspan="5" align="right" style="border:0"><a href="#">更多转让信息...</a></td></tr>';
		$html .= '</table>';
		return $html;
