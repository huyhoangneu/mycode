...
$py = new py_class();
$code = unserialize(file_get_contents('/data/grab/weathernew/new/code_weather.txt'));
foreach($code['other'] AS $ok => $ov)
{
		foreach($ov AS $id => $oinfo)
		{
				$other_codeinfo = explode('|', $oinfo);
				$other_codeinfo = str_replace(array('\r','\r\n'), '', $other_codeinfo);
				$other_cityname = trim($other_codeinfo['1']);
				$other_codeid = trim($other_codeinfo['0']);
				$other_filename = $other_cityname;
				$other_weather_info = getOtherInfo($other_codeid, $other_cityname);
				$other_weather_one = setTemplate($other_weather_info);
				echo WEATHER_PATH.urlencode($other_filename)."\n";
				if($other_weather_one) file_put_contents(WEATHER_PATH.urlencode($other_filename) . '.js', $other_weather_one, LOCK_EX);
				//if($other_weather_one) file_put_contents(WEATHER_PATH.urlencode($other_filename) . '.js', $other_weather_one, LOCK_EX);
				echo $other_filename."\n";
		}
}
foreach($code['china'] AS $k => $v)
{
	if(is_array($v))
	{
		$out = 0;
		foreach($v AS $name => $info)
		{
				if(is_array($info))
				{
						if($out == 0)
						{
								$piv_code = $info['0'];
								$piv_codeinfo = explode('|', $piv_code);
								$piv_codeinfo = str_replace(array('\r','\r\n'), '', $piv_codeinfo);
								$piv_cityname = trim($piv_codeinfo['1']);
								$piv_codeid = trim($piv_codeinfo['0']);
								$piv_flag = substr($piv_codeid, 0, 3);
								$piv_p_cityname = trim($piv_codeinfo['2']);//城市名称
								$piv_china_weather_infos = getChinaInfo($piv_codeid, $piv_cityname, $piv_p_cityname);
						}
					$out +=1;
					foreach($info AS $key => $one)
					{
							//内蒙古->赤峰->左旗
							$china_weather_info_1 = $china_weather_infos = '';
							$codeinfo = explode('|', $one);
							...
							if($key ==0)
							{
									$china_weather_infos_1 = getChinaInfo($codeid, $cityname, $p_cityname);
									if($k != '直辖市' && empty($china_weather_infos_1)) $china_weather_infos_1 = $piv_china_weather_infos;
									$china_weather_infos = $china_weather_infos_1;
									//$china_weather_infos = $china_weather_infos_1;
							}
							else
							{
								$china_weather_infos_2 = getChinaInfo($codeid, $cityname, $p_cityname);
								//print_r($china_weather_infos_2);
								if(empty($china_weather_infos_2))
								{	
										$china_weather_infos = $china_weather_infos_1;
								}
								else
								{	
										$china_weather_infos = $china_weather_infos_2;
								}
								//print_r($china_weather_infos);
								//exit;
								//if(empty($china_weather_infos_2)) $china_weather_infos = $china_weather_infos_1;
							}
							if(!$china_weather_infos){ echo "stop";exit; continue;}
							...
							$china_weather_infos['city']['0'] = $py->str2py($filename);
							$china_weather_infos['city']['1'] = $filename;
							//print_r($china_weather_infos);
							$weather_one = setTemplate($china_weather_infos);
							//echo $weather_one;
