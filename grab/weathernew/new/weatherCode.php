...
{//print_r($china_value);exit;
	...
	foreach ($china_city2['0'] AS $k2 => $v2) 
	{
		...
		foreach ($tmp3['0'] as $k4 => $v4) 
		{
			...
			if(!empty($tmp3['2'][$k4])) $code['china'][$name][$name1][] = $tmp3['1'][$k4].'|'.$tmp3['2'][$k4].'|'.$china_city2['2'][$k2];
		}
...
foreach ($other_city['1'] AS $other_key => $other_value)
{
        ...
        foreach ($other_city2['0'] as $other_city2_key => $other_city2_value)
		{
				...
				{ 
						...
						$code['other'][$other_p['1']][] = $other_city2['1'][$other_city2_key].'|'.$other_city2['2'][$other_city2_key];
				}
        }
}
file_put_contents('./code_weather.txt', serialize($code));
exit;
//print_r($code);exit;
/*对城市列表整理*/
